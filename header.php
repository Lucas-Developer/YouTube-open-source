<?php
	...
	if (isset($_POST["login"])) {
		...
		if (mysql_num_rows($verificar)<=0) {
			echo '<h3 id="erro">Informações de login inválidas!</h3>';
		}else{
			$user_info = mysql_fetch_assoc($verificar);
		...
		if (isset($_COOKIE["login"]) && isset($_COOKIE["login2"])) {
			...
			if (mysql_num_rows($verificar)>=1) {
				...
				<div class="menu">
					...
					<form>
						<input type="text" name="search" placeholder="Pesquisar vídeos e contas..."><input type="submit" name="pesquisar" value="Pesquisar">
					</form>
				</div>
				<?php
				$log = "sim";
				$get_info = mysql_fetch_assoc($verificar);
				if ($get_info["imagem"]=="" || $get_info["capa"]=="") {
					mysql_query("UPDATE users SET imagem='bg.jpeg', capa='bg.jpeg'");
				}
			}else{
				?>
				<div class="menu">
				...
				<form id="login" method="POST">
					...
					<input type="submit" name="login" value="Entrar"><p onclick="location.href = 'registar.php'">Não tenho conta</p>
				</form>
