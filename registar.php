<?php
	...
	if (isset($_POST["registar"])) {
		...
		if (mysql_num_rows($verificar)>0) {
			...
			$inserir = mysql_query("INSERT INTO users (nome,email,password) VALUES ('$nome','$email','$password')");
			if ($inserir) {
				$get_user = mysql_query("SELECT * FROM users WHERE email='$email' AND password='$password'");
